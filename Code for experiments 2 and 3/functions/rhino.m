...
%% Added by Rik for defacing project - ie keep normalisation same
try
    refMRI = S.refMRI;
catch
    refMRI = sMRI;
...
D.inv{1}.mesh = spm_eeg_inv_mesh(refMRI,2);
...
if S.useheadshape
    ...
    headshape_polhemus = [D.fiducials.pnt]; % D.fiducials.fid.pnt]; % removed by Rik
    ...
    [M_icp_inv,hsp_err] = rhino_icp(headshape_coreg', headshape_polhemus', S.multistart, ha);
    M_icp     = inv(M_icp_inv);
