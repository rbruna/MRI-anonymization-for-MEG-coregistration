...
for isub = 1: nsub
    ...
    % Gets the voxel-to-real-word transformation from the original image.
    ...
    for ana = 1: numel ( anas )
        ...
        D = S.D;
        D.fname = sprintf( '%s_meg_%s_spm.mat', sub, anas {ana} );
...
% Saves the result.
save ( 'FidErr_spm', 'fid_error', 'subs', 'UseRhino', 'RemoveNose' );
