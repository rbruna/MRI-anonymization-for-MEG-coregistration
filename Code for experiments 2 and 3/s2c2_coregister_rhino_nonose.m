...
%     rotate3d
    ...
    for ana = 1: numel ( anas )
        ...
        S = load ( fullfile ( D.path, D.fname ) );
        D = S.D;
        D.fname = sprintf( '%s_meg_nonose_%s_rhino.mat', sub, anas {ana} );
        save ( fullfile ( D.path, D.fname ), 'D' );
        D = spm_eeg_load ( fullfile ( D.path, D.fname ) );
        ...
        % Deletes the uncompressed image and the RHINO copy.
        delete ( sprintf ( '%s/MRI_%s/%s/rhino_r%s_ses-meg1_T1w_%s.nii', bids_dir, anas { ana }, sub, sub, anas { ana } ) );
        delete ( rT1file );
